...
task (){
	...
	BASE="$FOLDER/$TODAY-person-japan"
	RAW="$BASE-raw.csv"
	OUTPUT="$BASE.csv"
	...
	< $FOLDER/Statistics\ -\ Project\ Euler-j1.html scrape -be 'table#main_table > tbody>tr:not(:first-child)' | xml2json \
	...
	do
	< $FOLDER/Statistics\ -\ Project\ Euler-j${i}.html scrape -be 'table#main_table > tbody>tr:not(:first-child)' | xml2json \
	| jq -c '.html.body.tr[] | {id,rank:.td[0]."$t",solved:.td[3]."$t",level:.td[4].a."$t",language:.td[5].a."$t",last_solved:.td[1].table.tbody.tr.td[0].div.span."$t"}' \
